...
fcst_calibration_prlr <- function(ref, hcst, fcst) {
  result <- CSTools::QuantileMapping(
    exp = hcst,
    obs = ref,
    exp_cor = fcst,
    sdate_dim = "sdate",
...
fcst.cal <- Compute(
  wf,
  chunks = list(longitude = 20, latitude = 20),
  threads_load = 1,
  threads_compute = 1
)$output1
...
shape <- readRDS("resources/shp/health_region/hr_shapefile.rds") |>
  sf::st_transform(crs = 4326)

nc_files <- list(
  tas = paste0(
    "temp_files/calibrated_forecast_",
    syear, sprintf("%02d", mm), "_tas.nc"
  ),
  prlr = paste0(
    "temp_files/calibrated_forecast_",
    syear, sprintf("%02d", mm), "_prlr.nc"
  )
)
...
purrr::iwalk(nc_files, \(nc_file, var_name) {
  extract_forecast_nc(nc_file, var_name)
})
